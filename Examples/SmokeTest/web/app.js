...
function showError(msg) {
    ...
    console.error("[OCASmoke]", msg);
}

async function main() {
    try {
        setStatus("Loading WASM module...");
        const response = await fetch("OCASmoke.wasm");
        if (!response.ok) {
            throw new Error("Failed to load OCASmoke.wasm: " + response.status);
        }
