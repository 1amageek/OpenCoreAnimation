...
| Tiled-layer generation, request ownership, and immutable pixel cache | `Mutex<CATiledLayer.TileState>` | Same common declaration | Package has no Embedded product; no weaker branch exists | `beginTileRequest`, `cancelTileRequest`, `cacheImage`, cache reads, and invalidation helpers; image conversion occurs outside `withLock`, followed by a generation-matched insertion | Released with the owning tiled layer; invalidation drops cached storage and request ownership inside one critical section, with no callback, I/O, `await`, or GPU work under the mutex |
