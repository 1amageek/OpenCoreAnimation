...
| Evidence | Result |
|---|---|
| Native package | 680 tests passed |
...
| Native engine verification | The native `CAAnimationEngine` uses the real offscreen `CAMetalRenderer`, lazily creates a destination from the root bounds, submits a command buffer, records synchronous failures explicitly, and clears dirty state only after submission. Display-link ticks skip submission for clean static trees, future animations, and paused animations after their committed state is drawn. Dirty descendants, progressing animations, one terminal frame, and renderer-owned tile or emitter work submit; manual `renderFrame()` remains unconditional. Mask animations participate in the same submission decision, frame-rate arbitration, and completion traversal. The former no-op native renderer was removed; native tests verify submission decisions and read the submitted green pixel from the engine-owned Metal texture. Immutable commit-owned snapshots and active-subtree-only presentation evaluation remain open Phase 4 work |
