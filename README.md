...
| Tiled rendering | Static `CATiledLayer` trees capture geometry and a `Sendable` value-owned content snapshot at commit, then use renderer-owned generation, request, immutable pixel-cache, fade, and WebGPU texture state without retaining or reading the model layer. Bounds, scale, tile geometry, LOD, and display invalidation advance a synchronized generation, so stale asynchronous completion is rejected before pixel conversion and cannot overwrite newer content, report an irrelevant conversion failure, or clear a replacement request. Conversion stays outside the tile-state lock and insertion revalidates ownership atomically. Immutable storage copies preserve an O(1) GPU cache identity, avoiding a complete pixel hash on every frame. Provider capture/drawing, fade timing, geometry, capacity, context/image creation, conversion, and renderer-resource failures remain typed instead of becoming empty tiles. Native tests cover immutable provider capture, unsafe-provider rejection, LOD equivalence, synchronized compatibility state, stale malformed results, storage identity, and presentation identity; Chromium readback proves committed red pixels survive a later provider mutation and change to blue only after explicit invalidation |
...
| Metal renderer contract | `CARenderer(mtlTexture:options:)` accepts the documented color-space and client command-queue keys without trapping. Unknown keys, invalid values, queue/device mismatches, non-render-target textures, color-conversion failures, and incompatible destination changes remain observable through `lastRenderError`; failed destination replacement preserves the prior complete configuration. Every submitted layer owns immutable vertex and uniform buffers through command submission instead of rewriting one shared allocation between encoded draws. Native Metal readback verifies distinct parent/child colors and geometry, requested sRGB output, client-queue identity, atomic destination replacement, and explicit rejection of layer semantics the verification backend cannot encode |
...
| Transaction completion | Completion blocks for property, hierarchy, mask, and display mutations are coordinated across nested transactions and distinct render roots. A renderer releases each render obligation only after command submission and committed dirty-state clearing; animation obligations remain independently outstanding until their animations stop. Completion callbacks can mutate the tree without those new dirty bits being erased by the preceding frame. Native tests cover disabled actions, explicit and implicit animations, groups, removal, detached masks, multiple mutation categories, and callback reentrancy. Static trees publish immutable snapshots, while animated trees publish a model-independent evaluator that owns copied layer state, animation graphs, every path command, image bytes, transition participants, effects, replicators, emitters, and tiled inputs behind one `Mutex<CALayer>`. Mutable `CGPath` instances are normalized into finite `Sendable` command arrays and materialized only inside the evaluation owner, so later source-path mutation cannot cross the commit boundary. Every frame produced by that evaluator is again a value-only snapshot, so WebGPU encoding never reads the mutable model tree. Invalid transition filter/type/subtype configurations remain attached to their exact snapshot node, allowing unaffected siblings to continue while diagnostics stay typed and deduplicated. Capture and encoding failures restore renderer-owned state and retain transaction obligations for retry |
| Native engine verification | The native `CAAnimationEngine` uses the real offscreen `CAMetalRenderer`, lazily creates a destination from the root bounds, submits a command buffer, records synchronous failures explicitly, and clears dirty state only after submission. Outermost transactions resolve parent-to-child layout to a fixed point before publishing either a CALayer-free value snapshot or a mutex-protected committed animation evaluator; layout callbacks that add or resize descendants are therefore captured in the same committed generation. Metal consumes the snapshot features it supports and returns `unsupportedCommittedSnapshotFeature(.imageContents)`, `.contentMask`, `.groupOpacity`, `.rasterization`, `.filters`, `.backdropComposition`, `.shadow`, `.gradient`, `.shape`, `.text`, `.transformDepth`, `.replicatorInstances`, or `.emitter` for committed resources it cannot encode instead of silently omitting them. Its recursive encoder reads only snapshot nodes. Node revision matching prevents an older submission from clearing a mutation made after capture, while commit generation matching prevents an older frame from acknowledging newer state. Native readback captures green, mutates the model to red, submits green, and proves that the red mutation remains dirty for the next commit. Device-gray backgrounds are converted to device RGB, while non-finite geometry, colors, rasterization scale, filters, gradients, shapes, text, transforms, replicators, emitters, and shadows remain typed committed failures instead of becoming successful fallback pixels. Display-link ticks skip submission for clean static trees, future animations, and paused animations after their committed state is drawn. Dirty descendants, progressing animations, one terminal frame, and renderer-owned tile or emitter work submit; manual `renderFrame()` remains unconditional. Detached masks participate in the same cycle-safe layout, submission, frame-rate, and completion traversal as sublayers. Active-subtree-only presentation evaluation and complete QuartzCore parity remain open |
| Native tiled verification | The Metal verification backend rejects committed tiled content with `unsupportedCommittedSnapshotFeature(.tiledLayer)` rather than omitting it and reporting success |
...
| Filters and transitions | Sibling and nested `CAFilter` chains use per-layer WebGPU resources with browser pixel and cleanup evidence. The transition snapshot renderer interpolates immutable premultiplied source/target RGBA in one fragment pass, preserving translucent and transparent pixels under stencil and true-3D depth. Its source and target subtrees, geometry, contents scale, filter name, filter parameters, and progress are value-owned, so later model-layer or filter mutation cannot alter that captured frame. Normal active-transition commits use the same committed animation evaluator and immutable snapshot renderer. Device-limited pixel extent, Float-safe projection, filter setup, progress, and dispatch preserve participant-aware typed failures instead of returning `nil` or a count alone. Unsupported configuration is isolated to its owning snapshot node, does not allocate another transition texture set, and does not prevent later sibling transitions from being evaluated. A transition preparation failure submits the unaffected current tree to remove stale participant pixels but keeps the committed state pending, preserves dirty state, and withholds completion instead of acknowledging that partial frame; preparation is retried, while an unchanged typed transition diagnostic is counted once until the transition changes or disappears. Composite resources, Float-safe bounds/offsets, opacity, vertex capacity, and pipeline selection are also typed; directional source/target allocations are reserved as one batch before either draw, and every successful path restores the base pipeline. All 7 executable OpenCoreImage transition pipelines have filter-specific browser pixel evidence, while unsupported transition filter objects, unknown built-in transition types, and unknown directional subtypes are rejected with their exact reason and without target-image or `.fromLeft` fallback |
