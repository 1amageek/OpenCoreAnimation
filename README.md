...
| Compositing filters | Executable two-input `CIFilter` operations receive a draw-order-accurate backdrop captured immediately before the layer. Consecutive operations feed the preceding composite into the next backdrop, later siblings remain on top, and the cumulative snapshot replaces rather than re-blends the framebuffer. `backgroundFilters` execute mixed `CAFilter`/`CIFilter` stages against that backdrop and are restricted by the transformed rounded layer shape. Straight/premultiplied alpha boundaries are explicit, and failed paths never fall through to source-over. Browser pixels cover multiply, screen, translucent backdrop replacement, mixed background-filter stages, rounded-mask coverage, ordered chaining, and resource eviction |
