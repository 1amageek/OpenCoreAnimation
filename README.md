...
| Evidence | Result |
|---|---|
| Native package | 803 tests passed |
...
| Tiled rendering | Static `CATiledLayer` trees capture geometry and a `Sendable` value-owned content snapshot at commit, then use renderer-owned generation, request, pixel-cache, fade, and WebGPU texture state without retaining or reading the model layer. Bounds, scale, tile geometry, LOD, and display invalidation advance a synchronized generation, so stale asynchronous completion cannot overwrite newer content or clear a replacement request. Provider capture/drawing, fade timing, geometry, capacity, context/image creation, conversion, and renderer-resource failures remain typed instead of becoming empty tiles. Native tests cover immutable provider capture, unsafe-provider rejection, LOD equivalence, synchronized compatibility state, and presentation identity; Chromium readback proves committed red pixels survive a later provider mutation and change to blue only after explicit invalidation |
