...
| Evidence | Result |
|---|---|
| Native package | 407 tests passed |
...
| Value-function animations | `CAValueFunction` transforms scalar basic and keyframe values after linear, discrete, or cubic evaluation. Non-additive animations replace the model transform, while additive animations concatenate with it; cumulative repeats convert their terminal scalar before accumulation. Native tests cover integer and floating-point inputs, and browser pixels verify keyframe translation reaches WebGPU rendering |
