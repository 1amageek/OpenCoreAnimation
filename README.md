...
| Evidence | Result |
|---|---|
| Native package | 751 tests passed |
| Browser | 3 checks passed, including immutable committed layer, compositing, background filters, scale-correct rasterization, and true-3D transform containers; float16 extended-dynamic-range color and `CGImage` output with SDR restoration; straight-alpha image normalization and alpha-correct mipmaps with typed malformed/non-finite-storage rejection; contents geometry; rasterized/tiled pixels; frozen transition pairs; multiple shadows; shape fill-rule holes; trimmed/dashed strokes; axial/radial/conic and 12-stop gradients; depth-preserving emitters/replicators; and replicated background/border/shape/image/gradient pixels read back from WebGPU |
