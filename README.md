...
| Evidence | Result |
|---|---|
| Native package | 369 tests passed |
...
| Text truncation | `CATextLayer` measures rendered glyph width through Canvas2D before applying `.start`, `.middle`, or `.end` ellipsis truncation. Extended grapheme clusters remain intact, wrapped overflow truncates the final visible line, unknown modes do not silently fall back to a different mode, and the texture cache varies by truncation mode. Browser pixels verify all three placements, wrapped truncation, and mode-change cache invalidation |
