...
| Compositing filters | Executable two-input `CIFilter` operations receive a draw-order-accurate backdrop captured immediately before the layer. Consecutive operations feed the preceding composite into the next backdrop, later siblings remain on top, straight/premultiplied alpha boundaries are explicit, and failed paths never fall through to source-over. Browser pixels cover multiply, screen, translucent coverage, ordered chaining, and resource eviction |
