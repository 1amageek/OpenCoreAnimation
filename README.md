...
| Evidence | Result |
|---|---|
| Native package | 688 tests passed |
...
| Native engine verification | The native `CAAnimationEngine` uses the real offscreen `CAMetalRenderer`, lazily creates a destination from the root bounds, submits a command buffer, records synchronous failures explicitly, and clears dirty state only after submission. Before encoding, Metal captures a `Sendable`, CALayer-free value snapshot containing all presentation input that backend consumes; its recursive encoder reads only snapshot nodes. A native readback test captures green, mutates the model to red, and still observes green, proving frame-boundary mutation isolation. Device-gray backgrounds are converted to device RGB, while non-finite geometry and invalid colors fail with typed renderer errors instead of becoming successful fallback pixels. Display-link ticks skip submission for clean static trees, future animations, and paused animations after their committed state is drawn. Dirty descendants, progressing animations, one terminal frame, and renderer-owned tile or emitter work submit; manual `renderFrame()` remains unconditional. Detached masks participate in the same cycle-safe layout, submission, frame-rate, and completion traversal as sublayers. Commit-owned snapshot delivery, WebGPU snapshot migration, and active-subtree-only presentation evaluation remain open Phase 4 work |
