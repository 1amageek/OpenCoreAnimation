...
| Remaining boundary | Core Image transition types without executable WGSL remain unavailable. `backgroundFilters` combined with the target's arbitrary `CALayer.mask`, composition below an ancestor `CALayer.mask`, and composition below a `shouldRasterize` ancestor are explicitly rejected pending scoped content-mask/raster propagation; complete QuartzCore parity is not claimed |
