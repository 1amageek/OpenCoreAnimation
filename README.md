...
| Compositing filters | Executable two-input `CIFilter` operations receive a draw-order-accurate backdrop captured immediately before the layer. Layer opacity scales only the premultiplied source before that operation; the backdrop is not faded. Consecutive operations feed the preceding composite into the next backdrop, later siblings remain on top, and the cumulative snapshot replaces rather than re-blends the framebuffer. `backgroundFilters` execute mixed `CAFilter`/`CIFilter` stages against that backdrop and are restricted by the transformed rounded layer shape. Straight/premultiplied alpha boundaries are explicit, and failed paths never fall through to source-over. Browser pixels cover multiply, screen, source opacity, translucent backdrop replacement, mixed background-filter stages, rounded-mask coverage, ordered chaining, and resource eviction |
...
| Remaining boundary | Core Image transition types without executable WGSL remain unavailable. Nested backdrop scopes, replicator-contained compositions, composition below inherited opacity, and composition targets intersected by layer/ancestor masks are explicitly rejected pending the unified scoped-composition pass; complete QuartzCore parity is not claimed |
