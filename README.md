...
| Evidence | Result |
|---|---|
| Native package | 580 tests passed |
...
| Text rendering | `CATextLayer` validates string, font, geometry, scale, layout modes, and converted foreground color before entering Canvas2D. Unsupported values and missing browser measurements report typed renderer failures instead of becoming description strings or estimated sizes. Text textures remain transparent over the separate layer-background pass, use typed cache keys, and rasterize at `contentsScale` while preserving point-space geometry |
