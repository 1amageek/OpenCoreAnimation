...
| Remaining boundary | Core Image transition types without executable WGSL remain unavailable. Filter/compositing/transition effects inside the mask layer tree itself and composition below a `shouldRasterize` ancestor are explicitly rejected pending recursive mask-effect and raster-cache propagation; complete QuartzCore parity is not claimed |
