...
| Remaining boundary | Core Image transition types without executable WGSL remain unavailable. Composition targets intersected by layer/ancestor masks and composition below a `shouldRasterize` ancestor are explicitly rejected pending scoped mask/raster propagation; complete QuartzCore parity is not claimed |
