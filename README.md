...
| Evidence | Result |
|---|---|
| Native package | 422 tests passed |
...
| Constraint layout | `CAConstraintLayoutManager` solves each independent system of sibling and superlayer equations simultaneously, preserving unconstrained geometry and isolating inconsistent components instead of depending on sublayer or constraint order. Bounds, constraint, layout-manager, and hierarchy mutations invalidate layout, and `CAAnimationEngine` resolves pending parent-to-child layout before rendering. Native tests cover nonzero bounds origins, coupled edge sizing, reversed sibling chains, missing sources, conflicts, and automatic render-time layout; browser pixels verify the solved sibling frames reach WebGPU rendering without a manual layout call |
