// Zero-dep static server for the OpenCoreAnimation smoke-test.
// Serves Examples/SmokeTest/web/* plus the freshly built OCASmoke.wasm.
//
// build.sh lays the artifact out as:
//   Examples/SmokeTest/web/         -> index.html, app.js, runtime.mjs (checked in)
//   Examples/SmokeTest/web/OCASmoke.wasm (produced by build.sh, gitignored)
// This server mounts that directory root-level so `fetch('OCASmoke.wasm')`
...
async function preflight() {
    const wasmFile = join(WEB_ROOT, "OCASmoke.wasm");
    try {
        await stat(wasmFile);
    } catch {
        console.error(
            `\n✗ OCASmoke.wasm missing at ${wasmFile}\n` +
            `  Build it first:\n` +
            `    ./build.sh\n` +
            `  (or manually: cd ../../Examples/SmokeTest && \\\n` +
            `    swift build --product OCASmoke --swift-sdk swift-6.3.1-RELEASE_wasm -c release && \\\n` +
            `    cp .build/wasm32-unknown-wasip1/release/OCASmoke.wasm web/)\n`
        );
        process.exit(1);
    }
}
...
createServer(serve).listen(PORT, "127.0.0.1", () => {
    console.log(`OpenCoreAnimation E2E server listening on http://127.0.0.1:${PORT}`);
