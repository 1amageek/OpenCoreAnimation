...
test.describe("OpenCoreAnimation smoke", () => {
    ...
    test("rendering: WebGPU readback contains layer colors", async ({ harness }) => {
        ...
        await expect.poll(() => h.getCornerCurveProbeResult(), { timeout: 10_000 }).toBe(
            "79,20,29,255;48,207,0,255;0,48,207,255;255,255,0,255;243,48,219,255;5,212,214,255;255,0,255,255;0,0,0,255,failures=2,typed=layer:future-curve,maskFailures=1,maskTyped=contentMask:future-mask-curve"
        );
        ...
        await expect.poll(() => h.getLayerFilterProbeResult(), { timeout: 10_000 }).toBe(
            "128,255,255,255;191,191,0,255;255,0,255,255;255,0,0,255;group=true,ungrouped=true,translucentGroup=true,translucentUngrouped=true,rejected=true,invalid=true,typed=true,alphaFilter=true,alphaPixel=13,141,147,255,displayTyped=true"
        );
        expect(await h.getActiveFilterResourceCount()).toBe(0);
        expect(await h.getLayerFilterFailureCount()).toBe(4);
        ...
        await expect.poll(() => h.getTransformDepthProbeResult(), { timeout: 10_000 }).toBe(
            "crossing=true,transparent=true,isolated=true,flattened=true,nested=true,captures=10,composites=10,groupOpacity=true,filter=true,mask=true,directMask=true,rasterMask=true,maskUpdated=true,nestedFilter=true,shadow=true,shadowPath=true,compositionDepth=true,nestedComposition=true,overflow=true,updated=true,reused=true,rasterFailure=true,depthFailures=0,frameFailures=0,resizeTyped=true"
        );

        await h.beginCompositionProbe();
        await expect.poll(() => h.getCompositionProbeResult(), { timeout: 10_000 }).toBe(
            "ordered=true,unbounded=true,maskBackdrop=true,typed=true,pixels=0,0,0,255;255,255,0,255;128,0,0,255;0,0,255,255;0,255,255,255;255,0,0,255;6,133,10,191;128,0,0,255;255,255,0,255;255,0,0,255;255,0,0,255;255,128,0,255;128,0,0,255;128,128,0,255;255,0,255,255;255,255,0,255;255,0,0,255;255,0,0,255;255,0,0,255;0,255,255,255;255,0,0,255;255,0,0,255;255,128,0,255;255,0,0,255;0,255,255,255;255,0,0,255;0,255,255,255;0,255,255,255;9,9,78,160;53,10,16,149,failures=1,after=0"
        );
        expect(await h.getCompositionFilterFailureCount()).toBe(1);
