...
private final class CAEmitterCellLayerReference: Sendable {
    // Swift's weak-reference runtime owns atomic zeroing. The reference is
    // assigned exactly once here, every collection access is Mutex-protected,
    // and callbacks are copied out before invoking layer code.
    private nonisolated(unsafe) weak var storedLayer:
        CAEmitterLayer?
...
open class CAEmitterCell: CAMediaTiming {
    ...
    private let owningLayerReferences =
        Mutex<[CAEmitterCellLayerReference]>([])
    ...
    internal func addOwningLayer(_ layer: CAEmitterLayer) {
        owningLayerReferences.withLock { references in
            references.removeAll { $0.layer == nil }
            guard !references.contains(where: { $0.layer === layer }) else {
                return
            }
            references.append(
                CAEmitterCellLayerReference(layer: layer)
            )
        }
    }

    internal func removeOwningLayer(_ layer: CAEmitterLayer) {
        owningLayerReferences.withLock { references in
            references.removeAll {
                $0.layer == nil || $0.layer === layer
            }
        }
    }

    private func notifyOwningLayers() {
        let layers = owningLayerReferences.withLock { references in
            references.removeAll { $0.layer == nil }
            return references.compactMap(\.layer)
        }
        for layer in layers {
            layer.emitterCellDidMutate()
        }
    }
