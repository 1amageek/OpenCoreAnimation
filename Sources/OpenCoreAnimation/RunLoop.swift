...
//  OpenCoreAnimation
//
//  Browser RunLoop identity and mode model for display-link scheduling.
//
...
/// A browser run-loop identity used to register display links by mode.
///
...
/// compiles without modification. Mode registrations remain distinct even though
/// JavaScript schedules all callbacks through one browser event loop.
///
/// ## Usage
///
/// ```swift
...
/// ```
///
/// - Note: Modes control registration lifetime, while frame delivery uses
///   `requestAnimationFrame` for every registered mode.
public final class RunLoop: Sendable {
    ...
    ///
    /// Modes identify independent display-link registrations. The browser does not
    /// expose Foundation's current-mode state, so every retained mode receives rAF delivery.
